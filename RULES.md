...
*The same material is in the app under the `?` button, generated from the live
scoring tables — that version is authoritative if you edit them.*
...
- **Added kong replacement went to the wrong seat.** When a player declared an
  added kong (加槓) and nobody robbed it, the replacement draw was handed to the
  player on their right instead of the declarer — who then played on with a
  14th tile. Several claims later that seat could end up with five melds, no
  tiles, and no legal discard, which locks the table. Global tile counts stayed
  correct throughout (the tiles were real, just with the wrong person), so the
  original conservation check never saw it. The fuzz harness now also asserts
  a per-seat invariant, and `test/engine.test.mjs` covers the case directly.

## Bonus tiles

Flowers and seasons are replaced automatically the moment they are drawn, which
is the rule — there is no option to decline one. The app makes the draw visible
rather than silent: the player who drew it gets a card naming the tile, and the
table log names it for everyone else. The card auto-dismisses, deliberately: a
blocking acknowledgement would let one distracted phone stall the table.
