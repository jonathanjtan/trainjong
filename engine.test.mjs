...
function playRandom(variantId, seed, rounds = 1, maxHands = 24) {
  ...
  while (g.phase !== 'match-over' && hands < maxHands && guard++ < 40000) {
    ...
    if (g.phase === 'play') {
      ...
      else if (l.riichi?.length && r < 0.25) g.act(g.turn, { type: 'discard', tile: l.riichi[(Math.random() * l.riichi.length) | 0], riichi: true });
      else {
        const opts = l.discard || [];
        assert.ok(opts.length, `no legal discard for seat ${g.turn}`);
        const res = g.act(g.turn, { type: 'discard', tile: opts[(Math.random() * opts.length) | 0] });
