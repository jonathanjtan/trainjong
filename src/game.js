import {
  fullWall, shuffle, rng, counts, sortTiles, bonus, WIND_LABEL, LABELS,
} from './tiles.js';
...
export class Game {
  ...
  drawTile(seat, { rinshan = false, first = false } = {}) {
    ...
    if (this.v.bonusTiles && bonus(tile)) {
      ...
      // record WHICH tile, so the client can show the player what they picked
      // up rather than silently growing their bonus row
      this.bonusEvents = this.bonusEvents || [];
      this.bonusEvents.push({ n: ++this.eventNo, seat, tile, dealt: this.phase === 'deal' });
      if (this.bonusEvents.length > 24) this.bonusEvents.shift();
      this.push(`Seat ${seat + 1} draws ${LABELS[tile] || 'a bonus tile'}`);
      this.drawn = null;
  ...
  openClaims(from, tile, kind) {
    ...
    if (!any) {
      // A kong nobody can rob must go back to the DECLARER for the replacement
      // draw — afterClaims would hand it to the next seat, who then quietly
      // plays on with an extra tile. doKong does the replacement itself.
      if (kind === 'kong') return { ok: true, claims: false };
